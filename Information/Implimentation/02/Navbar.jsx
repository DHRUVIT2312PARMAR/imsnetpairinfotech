...
// ─── Constants ────────────────────────────────────────────────
const ROLE_STYLES = {
  super_admin: 'bg-red-100 text-red-600 border border-red-200',
...
const useNotifications = () => {
  ...
  // WebSocket for real-time push (falls back gracefully if WS not available)
  useEffect(() => {
    fetchNotifications();
    ...
    try {
        ...
        try {
          ...
          if (msg.type === 'NEW_NOTIFICATION') {
            ...
            toast.info(msg.notification.message, {
              icon: () => <i className={msg.notification.icon ?? 'ri-notification-3-line'} />,
              autoClose: 4000,
            });
          }
      ...
      ws.onerror = () => ws.close();
    } catch { /* WS not available — fall back to polling */ }
...
const ProfileDropdown = ({ user, onClose }) => {
  ...
  const menuItems = [
    ...
    { icon: 'ri-lock-password-line', label: t(lang, 'change_password'), path: '/settings/password' },
  ];
      ...
      <div className="relative px-4 pt-4 pb-3 border-b border-gray-100 dark:border-gray-800
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <img
              src={user?.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name ?? 'U')}&background=f97316&color=fff&size=48&bold=true`}
              alt={user?.name}
    ...
    `}>
      ...
      <div className="flex items-center gap-2.5 mr-3">
        <div className="w-8 h-8 rounded-lg overflow-hidden flex items-center justify-center
          ...
          <img
            src="/src/assets/imgs/Netpairlogo.png"
            alt="Netpair"
            className="w-7 h-7 object-contain"
            onError={(e) => {
              // Fallback: show NP text if image fails
      ...
      <div className="flex-1 min-w-0">
        <h1 className="text-sm font-bold text-gray-900 dark:text-white leading-tight truncate">
          {t(lang, 'welcome')}, <span className="text-orange-500">{user?.name?.split(' ')[0]}!</span>
        </h1>
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        ...
        <div className="relative" ref={profileRef}>
          <button onClick={openProfile}
            ...
            <div className="relative">
              <img
                src={user?.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name ?? 'U')}&background=f97316&color=fff&size=32&bold=true`}
                alt={user?.name}
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl
            ...
            text-white text-xs font-bold transition-colors shadow-sm"
        >
