...
const Policies = () => {
  ...
  const fetchAll = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await api.get("/policies?limit=100&sortBy=createdAt&order=desc");
  ...
  const handleAcknowledge = async (id) => {
    try {
      ...
      setPolicies(prev => prev.map(p =>
        p._id === id ? { ...p, acknowledged: data.data.acknowledged } : p
      ));
  ...
  return (
    <div className="relative h-full m-1 p-6 bg-gradient-to-br from-slate-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 flex flex-col gap-6 overflow-y-auto rounded-2xl">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "Total",   value: policies.length,                                       color: "from-blue-500 to-indigo-600",   icon: "ri-file-list-line" },
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-4 flex flex-col sm:flex-row gap-3">
        <input type="text" placeholder="Search policies..." value={search} onChange={e => setSearch(e.target.value)}
          className="flex-1 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 p-2.5 rounded-lg outline-none focus:border-blue-500 text-sm" />
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map(p => {
            const total = p.totalEmployees || 50;
            const acked = p.acknowledged || 0;
            return (
              <div key={p._id} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-5 flex flex-col gap-3 hover:shadow-md transition">
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="flex-1">
                    ...
                    <div className="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-1.5">
                      <div className="bg-blue-500 h-1.5 rounded-full transition-all" style={{ width: `${Math.min((acked / total) * 100, 100)}%` }}></div>
                    </div>
                  </div>
                  <button onClick={() => handleAcknowledge(p._id)}
          ...
          {filtered.length === 0 && !loading && (
            <div className="col-span-2 bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-12 text-center text-gray-400 dark:text-gray-500">
              ...
              <p className="text-sm">No policies found. {canManage ? "Add one above." : "Check back later."}</p>
            </div>
          )}
        </div>
      )}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-md p-6 shadow-2xl">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input className="w-full border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 p-2.5 rounded-lg outline-none focus:border-blue-500 mt-1 text-sm"
                  value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="e.g. Leave Policy" />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Category</label>
                <select className="w-full border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 p-2.5 rounded-lg outline-none mt-1 text-sm"
              ...
              <div>
                ...
                <textarea rows={3} className="w-full border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 p-2.5 rounded-lg outline-none focus:border-blue-500 mt-1 text-sm resize-none"
                  value={form.desc} onChange={e => setForm({ ...form, desc: e.target.value })} placeholder="Brief description..." />
              </div>
            </div>
            <div className="flex gap-3 mt-6">
              <button onClick={() => setOpen(false)} className="flex-1 py-2.5 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition">Cancel</button>
