...
const permissionMatrix = {
  ...
  admin:      ["employees:read", "attendance:manage", "leaves:approve", "reports:generate", "policies:write", "inventory:write", "roles:write", "audit-logs:read"],
  hr:         ["employees:write", "attendance:manage", "leaves:approve", "payroll:write", "projects:write", "assets:write", "wfh:manage"],
  employee:   ["profile:read/write", "attendance:read/write", "leaves:read/write", "tasks:read/write", "payroll:read (own)", "projects:read (assigned)"],
};
...
const RoleManagement = () => {
  ...
  const filtered = users.filter(u =>
    (`${u.firstName} ${u.lastName}`.toLowerCase().includes(search.toLowerCase()) ||
     (u.systemEmail || "").toLowerCase().includes(search.toLowerCase())) &&
          ...
          <tbody className="text-sm">
            {loading ? Array.from({ length: 5 }).map((_, i) => (
              <tr key={i} className="border-t animate-pulse">
                {Array.from({ length: 5 }).map((_, j) => <td key={j} className="px-4 py-3"><div className="h-3 bg-gray-200 rounded w-full"></div></td>)}
              </tr>
            )) : filtered.length === 0 ? (
              <tr><td colSpan={5} className="px-6 py-12 text-center text-gray-400 text-sm">No users found</td></tr>
            ) : filtered.map(u => (
              <tr key={u._id} className="border-t hover:bg-blue-50/40 transition">
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">{u.systemEmail}</p>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4 text-center text-gray-600">{u.department || "—"}</td>
                <td className="px-6 py-4 text-center">
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${roleStyle(u.role)}`}>{u.role}</span>
                </td>
                <td className="px-6 py-4 text-center">
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${u.isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>
            ...
            <div className="space-y-4">
              {Object.entries(permissionMatrix).map(([role, perms]) => (
                <div key={role} className="bg-gray-50 rounded-xl p-4">
                  <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${roleStyle(role)} mb-2 inline-block`}>{role}</span>
